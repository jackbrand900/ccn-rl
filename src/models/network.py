...
class ModularNetwork(nn.Module):
    def __init__(self,
        ...
        if use_cnn:
            if pretrained_cnn is not None:
                self.encoder = nn.Sequential(*list(pretrained_cnn.children())[:-1])
                with torch.no_grad():
                    dummy = torch.zeros(1, *input_shape).float()
                    if dummy.shape[1] != 3:
                        dummy = dummy[:, :3]
        ...
        if actor_critic:
            actor_layers = [nn.Linear(conv_out_dim, hidden_dim), nn.ReLU()]
            for _ in range(num_layers - 1):
                actor_layers.extend([nn.Linear(hidden_dim, hidden_dim), nn.ReLU()])
            actor_layers.append(nn.Linear(hidden_dim, output_dim))
            self.actor = nn.Sequential(*actor_layers)

            critic_layers = [nn.Linear(conv_out_dim, hidden_dim), nn.ReLU()]
            for _ in range(num_layers - 1):
    ...
    def select_action(self, state_tensor, context=None, constraint_monitor=None, deterministic=False):
        logits, value = self.forward(state_tensor, context=context) if self.actor_critic else (self.forward(state_tensor, context=context), None)
